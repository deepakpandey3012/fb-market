...
export class CampaignService {
  ...
  getDefaultAdSets(dateRange: string[], preSet) {
    if (dateRange.length != 0 && !preSet) {
      return this.http.get(
        `${this.getDefaultAdsetsUrl}?time_range={"since":"${dateRange[0]}","until":"${dateRange[1]}"}`
      );
    } else if (dateRange.length == 0 && preSet) {
      return this.http.get(`${this.getDefaultAdsetsUrl}?date_preset=${preSet}`);
    } else if (dateRange.length != 0 && preSet) {
      return this.http.get(
        `${this.getDefaultAdsetsUrl}?date_preset=${preSet}&time_range={"since":"${dateRange[0]}","until":"${dateRange[1]}"}`
      );
    }
  ...
  getAllAds(dateRange: string[], preSet) {
    if (dateRange.length != 0 && !preSet) {
      return this.http.get(
        `${this.getAllAdsUrl}?time_range={"since":"${dateRange[0]}","until":"${dateRange[1]}"}`
      );
    } else if (dateRange.length == 0 && preSet) {
      return this.http.get(`${this.getAllAdsUrl}?date_preset=${preSet}`);
    } else if (dateRange.length != 0 && preSet) {
      return this.http.get(
        `${this.getAllAdsUrl}?date_preset=${preSet}&time_range={"since":"${dateRange[0]}","until":"${dateRange[1]}"}`
      );
    }
  ...
  getAdPreview(adId: string) {
    return this.http.get(`${this.postAdsUrl}${adId}/preview`);
  }
